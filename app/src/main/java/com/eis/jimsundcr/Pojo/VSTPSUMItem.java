package com.eis.jimsundcr.Pojo;

import com.google.gson.annotations.SerializedName;

public class VSTPSUMItem{

	@SerializedName("rnetid")
	private String rnetid;

	@SerializedName("drcd")
	private String drcd;

	@SerializedName("RD3")
	private String rD3;

	@SerializedName("RD2")
	private String rD2;

	@SerializedName("mth3")
	private String mth3;

	@SerializedName("fees")
	private String fees;

	@SerializedName("mth2")
	private String mth2;

	@SerializedName("mth1")
	private String mth1;

	@SerializedName("drname")
	private String drname;

	@SerializedName("latesprec")
	private String latesprec;

	@SerializedName("noofwrkdays")
	private String noofwrkdays;

	@SerializedName("AC1")
	private String aC1;

	@SerializedName("AC2")
	private String aC2;

	@SerializedName("result")
	private String result;

	@SerializedName("AC3")
	private String aC3;

	@SerializedName("PC2")
	private String pC2;

	@SerializedName("PC1")
	private String pC1;

	@SerializedName("RC2")
	private String rC2;

	@SerializedName("PC3")
	private String pC3;

	@SerializedName("RC1")
	private String rC1;

	@SerializedName("RC3")
	private String rC3;

	@SerializedName("town")
	private String town;

	@SerializedName("conday")
	private String conday;

	@SerializedName("cntcd")
	private String cntcd;

	@SerializedName("AD1")
	private String aD1;

	@SerializedName("AD2")
	private String aD2;

	@SerializedName("AD3")
	private String aD3;

	@SerializedName("contime")
	private String contime;

	@SerializedName("noofpatient")
	private String noofpatient;

	@SerializedName("PD1")
	private String pD1;

	@SerializedName("PD3")
	private String pD3;

	@SerializedName("RD1")
	private String rD1;

	@SerializedName("stname")
	private String stname;

	@SerializedName("PD2")
	private String pD2;

	public void setRnetid(String rnetid){
		this.rnetid = rnetid;
	}

	public String getRnetid(){
		return rnetid;
	}

	public void setDrcd(String drcd){
		this.drcd = drcd;
	}

	public String getDrcd(){
		return drcd;
	}

	public void setRD3(String rD3){
		this.rD3 = rD3;
	}

	public String getRD3(){
		return rD3;
	}

	public void setRD2(String rD2){
		this.rD2 = rD2;
	}

	public String getRD2(){
		return rD2;
	}

	public void setMth3(String mth3){
		this.mth3 = mth3;
	}

	public String getMth3(){
		return mth3;
	}

	public void setFees(String fees){
		this.fees = fees;
	}

	public String getFees(){
		return fees;
	}

	public void setMth2(String mth2){
		this.mth2 = mth2;
	}

	public String getMth2(){
		return mth2;
	}

	public void setMth1(String mth1){
		this.mth1 = mth1;
	}

	public String getMth1(){
		return mth1;
	}

	public void setDrname(String drname){
		this.drname = drname;
	}

	public String getDrname(){
		return drname;
	}

	public void setLatesprec(String latesprec){
		this.latesprec = latesprec;
	}

	public String getLatesprec(){
		return latesprec;
	}

	public void setNoofwrkdays(String noofwrkdays){
		this.noofwrkdays = noofwrkdays;
	}

	public String getNoofwrkdays(){
		return noofwrkdays;
	}

	public void setAC1(String aC1){
		this.aC1 = aC1;
	}

	public String getAC1(){
		return aC1;
	}

	public void setAC2(String aC2){
		this.aC2 = aC2;
	}

	public String getAC2(){
		return aC2;
	}

	public void setResult(String result){
		this.result = result;
	}

	public String getResult(){
		return result;
	}

	public void setAC3(String aC3){
		this.aC3 = aC3;
	}

	public String getAC3(){
		return aC3;
	}

	public void setPC2(String pC2){
		this.pC2 = pC2;
	}

	public String getPC2(){
		return pC2;
	}

	public void setPC1(String pC1){
		this.pC1 = pC1;
	}

	public String getPC1(){
		return pC1;
	}

	public void setRC2(String rC2){
		this.rC2 = rC2;
	}

	public String getRC2(){
		return rC2;
	}

	public void setPC3(String pC3){
		this.pC3 = pC3;
	}

	public String getPC3(){
		return pC3;
	}

	public void setRC1(String rC1){
		this.rC1 = rC1;
	}

	public String getRC1(){
		return rC1;
	}

	public void setRC3(String rC3){
		this.rC3 = rC3;
	}

	public String getRC3(){
		return rC3;
	}

	public void setTown(String town){
		this.town = town;
	}

	public String getTown(){
		return town;
	}

	public void setConday(String conday){
		this.conday = conday;
	}

	public String getConday(){
		return conday;
	}

	public void setCntcd(String cntcd){
		this.cntcd = cntcd;
	}

	public String getCntcd(){
		return cntcd;
	}

	public void setAD1(String aD1){
		this.aD1 = aD1;
	}

	public String getAD1(){
		return aD1;
	}

	public void setAD2(String aD2){
		this.aD2 = aD2;
	}

	public String getAD2(){
		return aD2;
	}

	public void setAD3(String aD3){
		this.aD3 = aD3;
	}

	public String getAD3(){
		return aD3;
	}

	public void setContime(String contime){
		this.contime = contime;
	}

	public String getContime(){
		return contime;
	}

	public void setNoofpatient(String noofpatient){
		this.noofpatient = noofpatient;
	}

	public String getNoofpatient(){
		return noofpatient;
	}

	public void setPD1(String pD1){
		this.pD1 = pD1;
	}

	public String getPD1(){
		return pD1;
	}

	public void setPD3(String pD3){
		this.pD3 = pD3;
	}

	public String getPD3(){
		return pD3;
	}

	public void setRD1(String rD1){
		this.rD1 = rD1;
	}

	public String getRD1(){
		return rD1;
	}

	public void setStname(String stname){
		this.stname = stname;
	}

	public String getStname(){
		return stname;
	}

	public void setPD2(String pD2){
		this.pD2 = pD2;
	}

	public String getPD2(){
		return pD2;
	}

	@Override
 	public String toString(){
		return 
			"VSTPSUMItem{" + 
			"rnetid = '" + rnetid + '\'' + 
			",drcd = '" + drcd + '\'' + 
			",rD3 = '" + rD3 + '\'' + 
			",rD2 = '" + rD2 + '\'' + 
			",mth3 = '" + mth3 + '\'' + 
			",fees = '" + fees + '\'' + 
			",mth2 = '" + mth2 + '\'' + 
			",mth1 = '" + mth1 + '\'' + 
			",drname = '" + drname + '\'' + 
			",latesprec = '" + latesprec + '\'' + 
			",noofwrkdays = '" + noofwrkdays + '\'' + 
			",aC1 = '" + aC1 + '\'' + 
			",aC2 = '" + aC2 + '\'' + 
			",result = '" + result + '\'' + 
			",aC3 = '" + aC3 + '\'' + 
			",pC2 = '" + pC2 + '\'' + 
			",pC1 = '" + pC1 + '\'' + 
			",rC2 = '" + rC2 + '\'' + 
			",pC3 = '" + pC3 + '\'' + 
			",rC1 = '" + rC1 + '\'' + 
			",rC3 = '" + rC3 + '\'' + 
			",town = '" + town + '\'' + 
			",conday = '" + conday + '\'' + 
			",cntcd = '" + cntcd + '\'' + 
			",aD1 = '" + aD1 + '\'' + 
			",aD2 = '" + aD2 + '\'' + 
			",aD3 = '" + aD3 + '\'' + 
			",contime = '" + contime + '\'' + 
			",noofpatient = '" + noofpatient + '\'' + 
			",pD1 = '" + pD1 + '\'' + 
			",pD3 = '" + pD3 + '\'' + 
			",rD1 = '" + rD1 + '\'' + 
			",stname = '" + stname + '\'' + 
			",pD2 = '" + pD2 + '\'' + 
			"}";
		}
}